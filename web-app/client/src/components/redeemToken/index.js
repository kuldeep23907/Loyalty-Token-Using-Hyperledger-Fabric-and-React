...
const RedeemToken = (props) => {
    ...
    const redeemToken = () => {
        let token = localStorage.getItem('token');
        let url = "http://192.168.43.54:8090/token/redeem-token/middlemen";
        fetch(url, {
            method: "POST",
            headers: {
                Accept: "application/json",
            ...
            body: JSON.stringify({
                ...
                amount: amount,
                consumerId: currentGrantId,
                retailerId: currentTakeId,
                otp: otp
            }),
        })
            .then((results) => {
            ...
            .then((data) => {
                ...
                if (data.message === "Success") {
                    ...
                    props.history.push("/consumer/redeem-token");
                }
                setSuccessMsg(false);
                setAlert(true);
                    ...
                    <Spinner animation="grow" />
                        ...
                        >
                            <Form.Group controlId="formBasicGrantId">
                                <Form.Label style={{ color: "white" }}>Token Take ID</Form.Label>
                                <Form.Control
                                    type="text"
                                    placeholder={currentTakeId}
                                    readOnly
                                    value={currentTakeId}
                                />
                            </Form.Group>
                            ...
                            <Form.Group controlId="exampleForm.ControlSelect1">
                                ...
                                <Form.Control
                                    as="select"
                                    onChange={(event) => {
                                        setCurrentGrantId(event.target.value);
                            ...
                            <Form.Group controlId="formBasicAmount">
                                <Form.Label style={{ color: "white" }}>One time password</Form.Label>
                                <Form.Control
                                    type="text"
                ...
                <div>
                    ...
                    {successMsg ? (
                        <Alert variant="success" onClose={() => setSuccessMsg(false)} dismissible>
                            ...
                            <p>ID: {transaction.ID} <hr></hr>
                                Amount yet to be paid: {transaction.Amount} <hr></hr>
                                Created At: {transaction.Timestamp}</p>
